...
def requests_yandex_news():
    ...
    response = requests.get(f'{url}', headers=my_headers)
    root = html.fromstring(response.text)
    # замечания по работе: лучше изнутри наружу, так как порядок блоков новостей может изменится
...
# оптимизировать можно: на главной опросить ссылки, всю остальную информацию собрать на странице новости
